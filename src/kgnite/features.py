...
def starter_notebook(
    competition: str, data_dir: str, columns: list[str]
) -> dict[str, Any]:
    column_note = (
        ", ".join(columns)
        if columns
    ...
    return {
        ...
        "metadata": {
            ...
            "kgnite": {"competition": competition},
        },
        "nbformat": 4,
        "nbformat_minor": 5,
    }
...
def write_starter_notebook(
    competition: str, output: Path, data_dir: Path, *, force: bool = False
) -> Path:
    ...
    notebook = starter_notebook(competition, str(data_dir), columns)
